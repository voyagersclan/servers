...
STEAM_PORT_MAPPING="27016:27015"
...
if [ ! "$(docker ps -aq -f name=$CONTAINER_NAME)" ]; then
    ...
    if [ "$(expr substr $(uname -s) 1 10)" = "MINGW32_NT" ] || [ "$(expr substr $(uname -s) 1 10)" = "MINGW64_NT" ]; then
        ...
        docker run -it -v /opt/mounts:/opt/hl2dm/mounts --name $CONTAINER_NAME -p "$STEAM_PORT_MAPPING/tcp" -p "$STEAM_PORT_MAPPING/udp" $IMAGE_NAME
    fi
fi
